...
Given the strengths and limitations of these two methodologies, we argue that future research on VLMs should aim to combine both perspectives. Macro-level findings should be traced back to micro-level analyses to uncover the mechanisms behind observed phenomena and link empirical findings to theoretical principles. Conversely, micro-level insights should be contextualized within the larger framework of VLM behavior, leveraging interpretability tools to address specific challenges. Some recent works have begun bridging these approaches. For example, one study linked poor VLM performance to limitations in CLIP’s embedding space by using interpretability tools to analyze it. Another study showed that classification errors were not due to information loss but rather insufficient training data, as demonstrated through probing techniques. However, these efforts remain nascent, and there is a pressing need to formalize this integration. By reviewing advancements in both areas, we aim to highlight this critical gap and call for future work to develop principled methodologies that deepen our understanding of VLMs and enable the creation of more robust models.


<!-- Vision-Language Models (VLMs) are a significant development in artificial intelligence, enabling the integration of visual and textual data processing. These models, which process images and text to generate outputs, have advanced rapidly in recent years. Systems like GPT-4V, LLaVA, OpenFlamingo, and BLIP demonstrate capabilities across tasks such as image captioning, visual question answering, and multimodal reasoning. These advances support applications ranging from accessibility tools for the visually impaired to robotics, medicine, and creative industries.

Despite their achievements, the understanding of VLMs has not kept pace with their performance improvements. While these models perform well on benchmarks, they face unresolved challenges, including hallucinations, domain-specific biases, and limited generalization to unseen data. For instance, a model may describe objects not present in an image (hallucination) or fail to adapt to new contexts, raising reliability concerns in critical applications like medical imaging or autonomous systems.

These limitations are partly due to the field’s reliance on empirical progress. Current advancements stem from iterative improvements in architecture, training methods, and scaling, often guided by trial and error rather than a systematic understanding of how these models function. This lack of foundational insight hampers efforts to address their limitations and risks stalling future innovation.

Addressing this gap requires a scientific approach that examines both the behaviors and mechanisms of VLMs. This can be achieved by studying these models from two perspectives: macro-level behavioral analysis and micro-level mechanistic interpretability.

### Two Levels of Understanding

Macro-level analysis examines VLM performance across tasks, designs, and datasets. This perspective studies the influence of architectural choices, training strategies, and evaluation methods on outcomes. For example, it can explore how pretraining or fine-tuning impacts reasoning in novel visual contexts. Such studies identify patterns and bottlenecks but often lack explanations for the underlying mechanisms.

Micro-level analysis investigates the internal workings of VLMs, focusing on how neurons, features, and representations process information. Techniques like probing and activation patching are used to uncover how specific components contribute to behavior. While this research reveals insights into how visual and textual data are integrated, it often struggles to translate these findings into practical improvements.

### Bridging the Perspectives

Integrating macro- and micro-level insights can provide a more comprehensive understanding of VLMs. Macro-level findings can guide micro-level investigations into specific mechanisms, while micro-level insights can explain observed behaviors. For instance, macro-level studies might identify an architectural feature that improves reasoning, while micro-level research can reveal the processes enabling this improvement. This synergy can inform the design of more robust, interpretable, and generalizable models.

Collaboration between these perspectives can address challenges neither can solve independently. Macro-level research often grapples with the “black box” nature of VLMs, while micro-level studies risk losing sight of system-level behavior. Bridging these gaps requires connecting behavioral observations with mechanistic explanations.

### The Importance of Timing

Developing a scientific understanding of VLMs is urgent as these models are increasingly deployed in critical applications. Limitations like hallucinations, brittleness, and opacity pose risks that cannot be mitigated through performance metrics alone. The rapid innovation in VLM architectures and the availability of open-source models provide an opportunity to deepen understanding and guide principled advancements.

This blog explores how macro- and micro-level perspectives can complement each other to advance VLM research. We review current progress, challenges, and opportunities, advocating for a scientific approach that not only drives innovation but ensures models are reliable, interpretable, and aligned with societal needs. -->


<!-- Vision-Language Models (VLMs) represent a revolutionary advancement in artificial intelligence, bridging the gap between visual and textual data processing. These models, which take both images and text as input to generate meaningful outputs, have rapidly evolved in recent years. Flagship systems such as GPT-4V, LLaVA, OpenFlamingo, and BLIP have demonstrated exceptional capabilities across diverse tasks, including image captioning, visual question answering, multimodal reasoning, and more. Such progress is transforming various domains, from accessibility technologies like image-based content descriptions for the visually impaired to cutting-edge applications in robotics, medicine, and creative industries.

However, as with any disruptive technology, the excitement surrounding VLMs has outpaced our scientific understanding of their inner workings. While these models excel on standardized benchmarks, significant challenges remain unresolved, including their tendency to hallucinate, their susceptibility to domain-specific biases, and their limited ability to generalize robustly to unseen data. For instance, a model might confidently generate a caption describing objects not present in an image (hallucination) or fail to adapt its reasoning to contexts outside its training data. These issues are not merely academic; they undermine the reliability of VLMs in critical applications, such as medical imaging or autonomous vehicles, where errors could have serious consequences.

A major reason for these limitations lies in the nature of progress in the field. Much of the recent success in VLMs has been driven by engineering efforts, focusing on iterative improvements to architecture design, training recipes, and scaling strategies. While these innovations have led to rapid empirical advances, they are largely empirical, relying on trial and error rather than guided by a deep understanding of how VLMs actually work. This lack of scientific insight not only hinders further progress but also makes it challenging to identify principled solutions to address the known limitations of these models.

To address this gap, it is crucial to adopt a more scientific approach to VLM research, one that systematically investigates both what these models do and how they do it. This requires studying VLMs from two complementary perspectives: macro-level behavior analysis and micro-level mechanistic interpretability.

Two Levels of Understanding

At the macro level, researchers analyze the behavior of VLMs across diverse designs and tasks. This perspective involves studying how architectural choices, training strategies, and evaluation methodologies affect model performance on benchmarks. For example, macro-level studies might explore how pretraining on large-scale datasets or fine-tuning with domain-specific data impacts a model’s ability to reason about novel visual contexts. These investigations often draw inspiration from fields like behavioral science or ethology, where the focus is on describing patterns and identifying bottlenecks in model performance. However, while macro-level research excels at identifying correlations and generating hypotheses, it often falls short of explaining the underlying mechanisms driving these observed behaviors.

At the micro level, the focus shifts to understanding the internal workings of VLMs. Mechanistic interpretability investigates how a model’s neurons, features, and internal representations encode and process information. Drawing parallels to fields like neuroscience, this perspective aims to dissect the “brain” of a VLM, using techniques like probing, activation patching, and sparse autoencoders to uncover how specific components contribute to the model’s overall behavior. Micro-level research provides valuable insights into how information flows through the layers of a VLM, how visual and textual data are integrated, and which internal representations drive the model’s predictions. However, despite its promise, micro-level research often lacks a direct connection to practical improvements in VLM design or deployment, limiting its broader impact.

The Need for Integration

The true potential of VLM research lies in bridging these two perspectives. By integrating macro-level insights with micro-level methods, researchers can move beyond descriptive analyses to develop a principled understanding of VLMs. For instance, macro-level studies might identify that a particular architectural design leads to better multimodal reasoning, while micro-level analyses could explain why this design works, revealing the specific mechanisms that enable improved reasoning capabilities. Such integration would not only enhance our understanding of VLMs but also provide actionable insights for designing more robust, interpretable, and generalizable models.

Moreover, fostering collaboration between these two research streams can address challenges that neither perspective can solve alone. Macro-level studies often struggle with the “black box” nature of VLMs, where observed behaviors cannot be linked to specific mechanisms. Micro-level research, on the other hand, can become too focused on isolated components, losing sight of the broader system-level behaviors. Bridging these gaps requires a more holistic approach, one that connects behavioral observations with mechanistic explanations to build a unified understanding of VLMs.

Why Now?

The need for a scientific understanding of VLMs is more urgent than ever. As these models become increasingly integrated into critical applications, their limitations—such as hallucination, brittleness, and opacity—pose significant risks. Addressing these issues requires moving beyond surface-level performance metrics to uncover the principles governing VLM behavior. Additionally, the rapid pace of innovation in VLMs, including the development of open-source models and the introduction of novel architectural paradigms, presents a unique opportunity to study these models in unprecedented detail.

In this blog, we aim to advance this goal by systematically reviewing the current state of VLM research through the lenses of macro-level behavior analysis and micro-level mechanistic interpretability. We explore how these perspectives can complement each other to provide a more comprehensive understanding of VLMs, highlighting the progress made, the challenges faced, and the opportunities ahead. Ultimately, we hope to inspire a shift toward a more scientific approach to VLM research, one that not only drives future innovations but also ensures these models are reliable, interpretable, and aligned with human needs.




- Introduction of VLMs: what are they, how good they are, why they are important
- Current research landscape of VLMs (many VLMs, improved standardized benchmarks) and why VLMs understanding are important (why hallucination, why ...)
- Two level of understanding (bottom-up and top-down)
- Macro-level: behavior analysis, more empirical, get good research questions but don't know why
- Micro-level: mechanistic interpretability, have many useful methods but lack of overall usefulness
- The importance of bridging insights across communities, to enhance VLM

Vision language models (VLMs) represent a large family of models that take both visual and textual inputs and generate textual outputs. Recently, there are significant advances in VLMs, such as private models like GPT-4o, Claude, Gemini or open-source models like BLIP, LLaVA, OpenFlamingo, etc. These models have shown remarkable capabilities in various tasks, such as image captioning, visual question answering, and multimodal reasoning. 

However, despite the rapid progress in VLMs, our understanding of these models remains limited. Most of the research in VLMs has focused on improving performance on standardized benchmarks, without a deep understanding of how these models work. This lack of scientific insight poses challenges for further enhancing the robustness, generalization, and interpretability of VLMs.

In this blog, we aim to provide a comprehensive review of existing works that make the VLM research more "sciencey." 
